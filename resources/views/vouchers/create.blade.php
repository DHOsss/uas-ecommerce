...
<div class="card" style="max-width:500px">
    <form action="{{ route('vouchers.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            @error('min_purchase') <div class="error">{{ $message }}</div> @enderror
        </div>
        <div class="form-group">
            <label>Kadaluarsa</label>
            <input type="date" name="expired_at" value="{{ old('expired_at') }}">
            @error('expired_at') <div class="error">{{ $message }}</div> @enderror
        </div>
        <div class="form-group">
            <label>
        ...
        <a href="{{ route('vouchers.index') }}" class="btn btn-primary">Batal</a>
    </form>
</div>
@endsection
